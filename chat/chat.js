...
function displayChats(chats) {
    ...
    const selectedChatId = currentChatPartner ? currentChatPartner.id : null;

    chatsList.innerHTML = chats.map(chat => `
        <div class="chat-partner ${selectedChatId === chat.id ? 'selected' : ''}" 
             data-friend-id="${chat.id}">
                ...
                <div class="chat-last-message" id="last-message-${chat.id}">
                    ${chat.last_message ? 
                        (chat.last_message_sender_id === currentUser.id ? 
                            'Вы: ' : '') + chat.last_message : 
                        'Нет сообщений'}
                </div>
            </div>
            <div class="chat-meta">
                <span class="status-indicator ${isOnline(chat.last_activity) ? 'online' : 'offline'}"></span>
                <span class="last-activity">${getLastActivityTime(chat.last_activity)}</span>
            </div>
        </div>
    `).join('');

    // Добавляем обработчики кликов
    document.querySelectorAll('.chat-partner').forEach(chatElement => {
        chatElement.addEventListener('click', () => {
            const friendId = chatElement.dataset.friendId;
            const friend = chats.find(c => c.id === parseInt(friendId));
            if (friend) {
                selectChat(friend);
            }
        });
