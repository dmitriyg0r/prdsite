...
// Добавляем маршруты для White List
app.get('/api/WhiteList', async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM white_list ORDER BY created_at DESC');
        ...
        console.error('Error loading White List:', err);
        res.status(500).json({
            success: false,
            error: 'Ошибка при загрузке White List'
        });
    }
});
...
app.post('/api/WhiteList', async (req, res) => {
    try {
        ...
        const result = await pool.query(
            'INSERT INTO white_list (UUID, user, created_at) VALUES ($1, $2, NOW()) RETURNING *',
            [UUID, user]
        );
        ...
        console.error('Error adding to White List:', err);
        res.status(500).json({
            success: false,
            error: 'Ошибка при добавлении в White List'
        });
    }
});
...
app.delete('/api/WhiteList/:uuid', async (req, res) => {
    try {
        ...
        await pool.query('DELETE FROM white_list WHERE UUID = $1', [uuid]);
        ...
        console.error('Error removing from White List:', err);
        res.status(500).json({
            success: false,
            error: 'Ошибка при удалении из White List'
        });
    }
});
